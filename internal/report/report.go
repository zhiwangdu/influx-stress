...
func Insert(name string, columns []string, values [][]interface{}, pointCount int, runtime time.Duration) string {
	ir := &insertReport{
		name:    name,
		columns: columns,
		values:  values,
	...
	ir.pointsPerSecond = int(float64(pointCount) / runtime.Seconds())
...
func Query(name string, columns []string, values [][]interface{}) string {
	qr := &queryReport{
		name:    name,
		columns: columns,
		values:  values,
...
func InfluxQL(statement string, columns []string, values [][]interface{}) string {
	iqlr := &influxQlReport{
		statement: statement,
		columns:   columns,
		values:    values,
